  ...
  async function loadData() {
    ...
    try {
      ...
      if (companies.length > 0) {
        ...
        const [agentsList, deptsList] = await Promise.all([
  ...
  return (
    <ShellLayout pageTitle="Agent Registry" breadcrumb="Organization">
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2 border-b border-[#1e2738]">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-400">
              Authoritative catalog of organizational AI agents, reporting relationships, and capability declarations.
            </p>
          </div>
        ...
        {activeCompany && (
          <>
            {/* Registry Stats Overview & Honest Presence Distinction */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="p-4 rounded-xl border border-[#1e2738] bg-[#111724]">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-slate-400 font-medium uppercase">Registry Active</span>
                  <Check className="w-4 h-4 text-emerald-400" />
                </div>
                <div className="text-2xl font-bold font-mono text-emerald-400">{activeCount}</div>
                <p className="text-[11px] text-slate-400 pt-1 font-mono">Available for execution assignment</p>
              </div>

              <div className="p-4 rounded-xl border border-[#1e2738] bg-[#111724]">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-slate-400 font-medium uppercase">Registry Inactive</span>
                  <Sliders className="w-4 h-4 text-amber-400" />
                </div>
                <div className="text-2xl font-bold font-mono text-amber-400">{inactiveCount}</div>
                <p className="text-[11px] text-slate-400 pt-1 font-mono">Disabled from assignment</p>
              </div>

              <div className="p-4 rounded-xl border border-[#1e2738] bg-[#111724]">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-slate-400 font-medium uppercase">Runtime Active</span>
                  <Bot className="w-4 h-4 text-slate-400" />
                </div>
                <div className="text-2xl font-bold font-mono text-slate-400">0</div>
                <p className="text-[11px] text-slate-400 pt-1 font-mono">Execution scheduled for Phase 5</p>
              </div>
            </div>
            ...
            {filteredAgents.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredAgents.map((agent) => (
                  <div
                    key={agent.id}
                    className="rounded-xl border border-[#1e2738] bg-[#111724] p-5 flex flex-col justify-between hover:border-slate-700 transition space-y-4"
                  >
                    <div>
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <button
                          ...
                          title="Click to toggle registry status"
                        >
                          {agent.status}
                        </button>
                      </div>
            ...
            <div className="w-full max-w-xl bg-[#0e1420] border-l border-[#1e2738] h-full overflow-y-auto p-6 flex flex-col justify-between">
              {detailLoading || !agentDetail ? (
                ...
                <div className="space-y-6">
                  ...
                  {/* Honest Operational Execution States (Phases 5 & 6) */}
                  <div className="space-y-2 pt-2 border-t border-[#1e2738]">
                    <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                      <FolderGit2 className="w-3.5 h-3.5 text-slate-400" />
                      <span>Execution Tasks & Metrics</span>
                    </h4>
                    <div className="p-3.5 rounded-xl bg-[#111724] border border-[#1e2738] text-xs text-slate-400 space-y-1 font-mono text-[11px]">
                      <div className="flex justify-between">
                        <span className="text-slate-500">Current Task:</span>
                        <span className="text-slate-400">None (Task Engine scheduled for Phase 6)</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Previous Tasks:</span>
                        <span className="text-slate-400">0 completed</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Success Rate:</span>
                        <span className="text-slate-400">Not tracked in Phase 4</span>
                      </div>
                    </div>
                  </div>
                </div>
